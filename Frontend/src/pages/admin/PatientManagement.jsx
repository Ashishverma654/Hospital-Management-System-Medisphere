  ...
  return (
    <section className="space-y-6">
      <div className="rounded-[2rem] bg-white p-8 shadow-sm">
        <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Patient Administration</p>
        <h2 className="mt-2 text-3xl font-semibold text-slate-900">Patient Console</h2>
      ...
      <article className="rounded-[2rem] bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Daily Patient Board</p>
            <h3 className="mt-2 text-2xl font-semibold text-slate-900">Operational activity by day</h3>
          </div>
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {summaryCards.map((item) => (
            <article key={item.label} className="rounded-[1.5rem] border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm text-slate-500">{item.label}</p>
              <p className="mt-3 text-3xl font-semibold text-slate-900">{boardLoading ? '...' : item.value}</p>
            </article>
          ))}
        </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.05fr,0.95fr]">
        <article className="rounded-[2rem] bg-white p-6 shadow-sm">
          <div className="flex flex-col gap-3 lg:flex-row">
            ...
            <select value={filterProfileStatus} onChange={(event) => setFilterProfileStatus(event.target.value)} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm outline-none focus:border-slate-900">
          ...
          <div className="mt-6 overflow-x-auto">
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {loading && (
                  <tr>
                    <td colSpan={7} className="px-3 py-10 text-center text-slate-500">Loading patients...</td>
                  </tr>
                )}
                {!loading && patients.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-3 py-10 text-center text-slate-500">No patients found.</td>
                  </tr>
                )}
                {patients.map((patient) => (
                  <tr key={patient.id} className="border-b border-slate-100 last:border-b-0">
                    <td className="px-3 py-3">
                      <p className="font-medium text-slate-900">{patient.user?.name || 'Patient'}</p>
                      <p className="text-xs text-slate-500">{patient.bloodGroup || 'Blood group not set'}</p>
                    </td>
                    <td className="px-3 py-3 font-mono text-xs text-slate-500">{patient.user?.patientId || patient.medicalRecordNumber || '—'}</td>
                    <td className="px-3 py-3 text-slate-600">
                      <p>{patient.user?.email || '—'}</p>
                      <p className="text-xs text-slate-500">{patient.user?.phone || 'No phone'}</p>
                    </td>
                    <td className="px-3 py-3 text-slate-600">
                    ...
                    <td className="px-3 py-3 text-slate-500">{new Date(patient.createdAt).toLocaleDateString()}</td>
                    <td className="px-3 py-3">
        ...
        <article className="rounded-[2rem] bg-white p-6 shadow-sm">
          ...
          {detailLoading && (
            <div className="flex min-h-[420px] items-center justify-center text-slate-500">Loading patient detail...</div>
          )}
          ...
          {detail?.patient && !detailLoading && (
            <div className="space-y-6">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  <p className="text-sm uppercase tracking-[0.24em] text-slate-500">Patient Detail</p>
                  <h3 className="mt-2 text-2xl font-semibold text-slate-900">{detail.patient.user?.name}</h3>
              ...
              <RelatedSection title="Clinical profile">
                ...
                <div className="mt-3">
                  <p className="text-sm font-medium text-slate-700">Chronic diseases</p>
              ...
              <RelatedSection title="Timeline foundation">
                <div className="space-y-3">
                  {(detail.timeline || []).slice(0, 12).map((item, index) => (
                    <div key={`${item.type}-${item.id || index}`} className="rounded-[1.25rem] border border-slate-200 p-4">
                      <div className="flex items-center justify-between gap-3">
                        <p className="font-semibold text-slate-900">{item.title}</p>
                        <span className="text-xs uppercase tracking-[0.18em] text-slate-500">{item.type}</span>
                      </div>
                      <p className="mt-2 text-sm text-slate-600">{item.description}</p>
                      <p className="mt-2 text-xs text-slate-500">{new Date(item.occurredAt).toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              </RelatedSection>
      ...
      {showEditor && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <form onSubmit={handleUpdate} className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-xl font-semibold text-slate-900">Edit Patient Profile</h3>
              <button type="button" onClick={() => setShowEditor(false)} className="text-slate-500 hover:text-slate-900">✕</button>
            </div>

            <div className="mt-5 grid gap-4 md:grid-cols-2">
              <Field label="Patient name"><input type="text" value={form.name} onChange={(event) => setForm((current) => ({ ...current, name: event.target.value }))} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-900" /></Field>
              <Field label="Email"><input type="email" value={form.email} onChange={(event) => setForm((current) => ({ ...current, email: event.target.value }))} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-900" /></Field>
              <Field label="Phone"><input type="text" value={form.phone} onChange={(event) => setForm((current) => ({ ...current, phone: event.target.value }))} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-900" /></Field>
              <Field label="Gender">
              ...
              <Field label="Date of birth"><input type="date" value={form.dateOfBirth} onChange={(event) => setForm((current) => ({ ...current, dateOfBirth: event.target.value }))} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-900" /></Field>
              ...
              <Field label="Profile status">
                <select value={form.profileStatus} onChange={(event) => setForm((current) => ({ ...current, profileStatus: event.target.value }))} className="w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-900">
                  <option value="active">Active</option>
                  <option value="incomplete">Incomplete</option>
                  <option value="archived">Archived</option>
                </select>
            ...
            <Field label="Admin notes" className="mt-4"><textarea value={form.notes} onChange={(event) => setForm((current) => ({ ...current, notes: event.target.value }))} className="min-h-[120px] w-full rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-slate-900" /></Field>
  ...
  return (
    <article className="rounded-[1.5rem] border border-slate-200 p-4">
      ...
      <div className="mt-4 space-y-3">
        {items.length === 0 && <p className="text-sm text-slate-500">No activity recorded.</p>}
        {items.slice(0, 6).map((item) => (
          <div key={item.id} className="rounded-[1rem] bg-slate-50 p-3">
            <p className="font-medium text-slate-900">{item.name || item.patient?.name || item.patientName || 'Patient activity'}</p>
            {secondaryKey && <p className="mt-1 text-sm text-slate-600">{item[secondaryKey] || '—'}</p>}
            <p className="mt-1 text-xs text-slate-500">{new Date(item[dateKey]).toLocaleString()}</p>
          </div>
        ))}
      </div>
    </article>
    ...
    <div className="mt-4">
      ...
      <div className="mt-2 space-y-2">
        ...
        {items.slice(0, 5).map((item) => (
          <div key={item._id} className="rounded-[1rem] border border-slate-200 p-3 text-sm text-slate-600">
            {renderItem(item)}
          </div>
        ))}
      </div>
